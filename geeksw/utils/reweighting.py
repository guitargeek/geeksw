...
def reweight1d(a, reference=1, bins=10, get_w_ref=False):
    ...
    ----------
    a : array_like
        The array with the variable we want to reweight.
    reference : scalar or array_like
        Either a scalar value if you want to reweight to a flat distribution,
        or an array for extracting the distribution to match.
    ...
    get_w_ref:  bool, optinal
        If set to true, weights for the reference distribution will also be
        returned.

    Returns
    -------
    w : array of dtype float
        The weights for the input array to match the reference arrays distribution.
    w_ref : darray of dtype float, optional
        The weights for the reference array, with are 1 in the binning and 0 outside.
